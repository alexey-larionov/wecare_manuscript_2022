---
title: "WES: BC11 crude counts, WECARE-NFFE"
...
# Explore AF
sum(bc11_variants.df$total_af <= 0.01, na.rm=T)
min(bc11_variants.df$total_af, na.rm=T)
max(bc11_variants.df$total_af, na.rm=T) # No rare variants with MAF<0.01 at AF > 0.99
sum(is.na(bc11_variants.df$total_af))
...
bc11_fiv.df <- bc11_variants.df[fiv,] %>% 
  select(ID,vep_SYMBOL,vep_Consequence,vep_IMPACT,
         CLNSIG,CLNREVSTAT,PolyPhen_call,SIFT_call,
         vep_CADD_PHRED,
...
write.table(bc11_fiv.df, 
            file.path(data_folder,"bc11_fiv_wes_wecare_nffe.txt"),
