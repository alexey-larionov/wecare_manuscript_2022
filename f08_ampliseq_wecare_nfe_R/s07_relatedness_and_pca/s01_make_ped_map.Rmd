---
title: "Ampliseq wecare NFFE: make PLINK dataset for PCA calculation"
...
Input data: 1,922 variants x 586 samples (197 NFFE, 194 UBC & 195 CBC)  
Output PLINK ped-map: 207 vars x 586 samples (197 NFFE, 194 UBC & 195 CBC)  
...
#dir.create(plink_ped_folder, recursive = T)
...
# Verify multiallelic variants

Included into the previously available flag 
...
# All currently detected multiallelic variants 
# are within the previously flagged Multiallelics
sum(variants.df$Multiallelic & multiallelic_variants)

# Total variants that were flagged as Multiallelic before filtering
sum(variants.df$Multiallelic)
all(variants.df$Multiallelic[multiallelic_variants])
...
# Check NA-s in init_AF
sum(is.na(variants.df$init_AF))

common_ampliseq <- 
  variants.df$init_AF > 0.05 & 
  variants.df$init_AF < 0.95
sum(common_ampliseq)

# Check NA-s in variants.df$vep_gnomAD_NFE_AF
sum(is.na(variants.df$vep_gnomAD_NFE_AF))

# Count definitely known common variants (explicitly not including NA-s)
common_gnomAD_NFE <- 
  variants.df$vep_gnomAD_NFE_AF > 0.05 & 
  variants.df$vep_gnomAD_NFE_AF < 0.95 &
  !is.na(variants.df$vep_gnomAD_NFE_AF)

# Count definitely known common variants
sum(common_gnomAD_NFE)

```

# HWE

Check HWE using library HardyWeinberg (in case HWE has changed during filtering)  
...
# Select variants (use common_gnomAD_NFE instead of common_ampliseq ?)
variants_for_PCA <- SNP & common_ampliseq & !variants.df$Multiallelic
...
# Clean-up
rm(variants_for_PCA, SNP, common_ampliseq, common_gnomAD_NFE,
   multiallelic_variants, hwe_violators, variants.df, genotypes.mx)
