...
#SBATCH --output=s04_check_QC.log
#SBATCH --qos=INTR
...
# Start message
echo "Started s04_check_QC: $(date +%d%b%Y_%H:%M:%S)"
echo ""
...
stats_folder="${data_folder}/s04_stats"
rm -fr "${stats_folder}"
mkdir "${stats_folder}"
...
# Files
vcf="${data_folder}/ampliseq_nfe_locID_MAflag_vqsr_hf_split_clean.vcf.gz"
stats_file="${stats_folder}/ampliseq_nfe_locID_MAflag_vqsr_hf_split_clean.vchk"
